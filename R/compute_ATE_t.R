#' Computes the bounds on the ATE on the dataset, at the period given in input,
...
#' ATE must be estimated depends on the individual (e.g. in event-studies models).
...
#' covariates w.r.t. which the ATE must be computed. If null, the ATE w.r.t. all
...
#'  - reducedSampleSize: the number of individuals used to estimate the ATE. The
#'    individuals that were not observed at the period for which the ATE should
...
#'    (sharp or outer, depending on Option) bounds on the ATE, for each covariate
#'    in selectX.
#'  - CI: a matrix of size |selectX| x 2 containing in each row the estimated
#'    confidence interval for the ATE, for the corresponding covariate in selectX.
...
#'    individual observations. They will be used to estimate the average ATE
#'    over all periods. They are organised as follows:
#'  - indl_estimates$indl_bounds_on_delta is a matrix of size n x 2 x dimX
#'    containing the estimated lower and upper bounds on the ATE, for each
...
#'    upper bounds on the ATE, for each individual observation. It is NA for the
#'    individuals unobserved at the requested period.
#'
#' @export
#'
# @examples
compute_ATE_t <- function(data, TEstim, estimators = env(), other_numbers = env(), selectX = NULL, Option = "quick", CIOption = "CI2", alpha = 0.05,  nbCores = 4) {
  ...
  # We will need to perform estimation separately for each of the selected covariate
  for (k in selectX) {

    # We compute the "tilded" values (difference or ratio of variables at each period v.s. at TEstim)
    # IMPORTANT : here, we keep using V = exp(X'beta) (not the same as v(x, beta) for the ATE), but
    # we do use Vtilde = exp(X'beta - v(x, beta)) where v(x, beta) is the formula for ATE.
    # Similarly, Xtilde = X - X_t + (1 - X_tk) e_k
    # Vt is indeed exp(v(x, beta))
    Xtk <- extractIndlPeriod(X[,, k], TEstim)
    Vt <- rep(0, dim(X)[1])
    Xtilde <- array(NA, dim(X))
    Vtilde <- matrix(0, dim(X)[1], dim(X)[2])
    for (l in 1:dim(X)[3]) {
      if (l != k) {
        Xtilde[,, l] <- X[,, l] - repmat(matrix(extractIndlPeriod(X[,, l], TEstim), ncol = 1), 1, dim(X)[2])
        Vt <- Vt + extractIndlPeriod(X[,, l], TEstim) * beta_hat[l]
      } else {
        Xtilde[,, l] <- X[,, l] - repmat(matrix(1 - extractIndlPeriod(X[,, l], TEstim), ncol = 1), 1, dim(X)[2])
        Vt <- Vt + (1 - extractIndlPeriod(X[,, l], TEstim)) * beta_hat[l]
      }
      Vtilde <- Vtilde + Xtilde[,, l] * beta_hat[l]
    }
    Vtilde <- exp(Vtilde)
    Vt <- exp(Vt)

    # We compute the straightforward, point-identified first term in the ATE
    firstTermATE <- extractIndlPeriod(Y, TEstim) * (2 * Xtk - 1)

    # Get the approximated polynomial for Omega(·, x, beta) * h(·, x, beta)
    # (beta factor ignored)
    out <- best_approx_poly(Vtilde, Xtilde, forATE = TRUE, signFactorCoeffshATE = (-1)^Xtk)
    ...
    if (Option == "quick") {
      ...
      Delta_hat <- mean(firstTermATE + p_vec)
      ...
      bias_sup <- mean(indl_bias_sup)
      average_bounds_on_delta[match(k, selectX),] <- c(Delta_hat - bias_sup, Delta_hat + bias_sup)

      # We save the individual estimates of delta_hat(X) and of the maximal bias at X
      indl_estimates[["indl_bounds_on_delta"]][keepIndls, 1, match(k, selectX)] <- firstTermATE + p_vec - indl_bias_sup
      indl_estimates[["indl_bounds_on_delta"]][keepIndls, 2, match(k, selectX)] <- firstTermATE + p_vec + indl_bias_sup
      ...
      infl_func_delta <- firstTermATE + p_vec
      ...
      for (l in 1:dimX) {
        infl_func_delta <-
          infl_func_delta + phi_b[, l] *
          mean(
            p_vec * (
              S * ifelse(rep(l, nbIndls) != k, extractIndlPeriod(X[,, l], TEstim), 1 - extractIndlPeriod(X[,, l], TEstim)) -
                rowSums(X[,, l] * V * C_S_minus_one_one_period_out, na.rm = TRUE) / extractIndlPeriod(C_S_tab, S + 1)
            ) +
              rowSums(T_minus_t_choose_S_minus_t_mat * derivCoeffsOmegaDbeta[,, l], na.rm = TRUE) * Vt^S / extractIndlPeriod(C_S_tab, S + 1)
          )
      }

      ### Save the individual influence functions
      indl_estimates[["indl_infl_func_delta"]][keepIndls, 1, match(k, selectX)] <- infl_func_delta
      indl_estimates[["indl_infl_func_delta"]][keepIndls, 1, match(k, selectX)] <- infl_func_delta
      ...
      std_delta <- sd(infl_func_delta_cluster, na.rm = TRUE) / sqrt(nbIndls)
      ...
      ### whole set.
      bCI3 <- (abs(beta_hat[k]) + qnorm(1 - 4 * alpha / 5) * estimators$beta_hat$var_b[k, k] / fullSampleSize) * mean(indl_bias_sup)

      ### Then compute the length of each CI
      length_CI2 <- 2 * std_delta * sqrt(qchisq(1 - alpha, df = 1, ncp = (bias_sup / std_delta)^2))
      length_CI3 <- 2 * std_delta * sqrt(qchisq(1 - alpha / 5, df = 1, ncp = (bCI3 / std_delta)^2))

      CI2[match(k, selectX), ] <- c(Delta_hat - length_CI2 / 2, Delta_hat + length_CI2 / 2)
      CI3[match(k, selectX), ] <- c(Delta_hat - length_CI3 / 2, Delta_hat + length_CI3 / 2)
      ...
      # Obtain the bounds on the ATE
      out <- bound_delta(focused_data, focused_estimators, other_numbers, Vt, coeffsOmega, topCoeffOmega, nbCores, firstTermATE)
      average_bounds_on_delta[match(k, selectX), ] <- out$average_bounds_on_delta
      indl_bounds_on_delta <- out$indl_bounds_on_delta
      indl_estimates[["indl_bounds_on_delta"]][keepIndls,, match(k, selectX)] <- indl_bounds_on_delta
      ...
      for (t in 0:Tmax) {
        ...
        for_deriv_indl_bounds_on_delta <-
          bound_delta( # The first argument is the same as data but individuals which are not observed at the period of interest are taken out
            ...
            topCoeffOmega,
            nbCores,
            firstTermATE)$indl_bounds_on_delta
      ...
      for (l in 1:dimX) {
        ...
        format_data(data_deriv, beta_hat + (1:dimX == l) * h_deriv)

        # We compute the Vt and the coefficients for the new beta
        Vtilde_deriv <- Vtilde * exp(h_deriv * Xtilde[,, l])
        if (l != k) {
          Vt_deriv <- Vt * exp(h_deriv * extractIndlPeriod(X[,, l], TEstim))
        } else {
          Vt_deriv <- Vt * exp(h_deriv * (1 - extractIndlPeriod(X[,, l], TEstim)))
        }

        out <- best_approx_poly(Vtilde_deriv, Xtilde, forATE = TRUE, signFactorCoeffshATE = (-1)^Xtk)
        ...
        for_deriv_average_bounds_on_delta <- colMeans(repmat(matrix(changeOfSign, ncol = 1), 1, 2) * bound_delta(data_deriv, focused_estimators, other_numbers, Vt_deriv, repmat(matrix(changeOfSign, ncol = 1), 1, Tmax + 2) * coeffsOmega_deriv, changeOfSign * topCoeffOmega_deriv, nbCores, firstTermATE)$indl_bounds_on_delta)

        deriv_beta[l,] <- (for_deriv_average_bounds_on_delta - average_bounds_on_delta[match(k, selectX),]) / h_deriv
      }
      ...
      # We save it
      indl_estimates[["indl_infl_func_delta"]][keepIndls,, match(k, selectX)] <- infl_func_delta

      # Estimate the asymptotic variance from the influence functions
      # and compute confidence intervals
      infl_func_delta_cluster <- matrix(NA, length(unique(clusterIndexes)), 2)
      infl_func_delta_cluster[, 1] <- rowsum(infl_func_delta[, 1], clusterIndexes, na.rm = TRUE)
      infl_func_delta_cluster[, 2] <- rowsum(infl_func_delta[, 2], clusterIndexes, na.rm = TRUE)

      cur_beta_is_not_null <- rep(NA, nb_var)

      std_bounds <- apply(infl_func_delta_cluster, 2, sd, na.rm = TRUE) / sqrt(nbIndls)
      IM_quantile <- compute_IM_quantile(alpha, average_bounds_on_delta[match(k, selectX),], std_bounds)

      ### This is a t-test to see if the beta corresponding to the current AME
      ### is null
      cur_beta_is_not_null <- sqrt(fullSampleSize * beta_hat[k]^2 / estimators$beta_hat$var_b[k, k]) > qnorm(1 - alpha/2)

      CI[match(k, selectX), ] <- average_bounds_on_delta[match(k, selectX),] + IM_quantile * c(-std_bounds[1], std_bounds[2])
      if (!cur_beta_is_not_null) {
        CI[match(k, selectX), 1] <- min(CI[match(k, selectX), 1], 0)
        CI[match(k, selectX), 2] <- max(CI[match(k, selectX), 2], 0)
      }
    }
  }
  ...
  names(out) <- c("Option", "reducedSampleSize", "computationTime", "estimatedATEbounds", "CI", "alt_CI", "indl_estimates")
