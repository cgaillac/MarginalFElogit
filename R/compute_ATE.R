...
#'
# @examples
compute_ATE <- function(data, estimators = env(), selectX = NULL, compute_T = "all", Option  = "quick", CIOption = "CI2", alpha = 0.05, nbCores = 4) {
  ...
  requested_ATE_outputs <- list()
  names_requested_ATE <- c()
  ...
  for (i in 1:length(compute_T)) {
    ...
    requested_ATE_outputs[[i]] <- compute_ATE_t(data, t, estimators, other_numbers, selectX, Option, CIOption, alpha, nbCores)
  }
  names_requested_ATE <- paste0("T_", compute_T)

  # Compute the average if we computed all periods
  if (compute_average) {
    requested_ATE_outputs[[length(compute_T) + 1]] <- compute_average_AMTE(requested_ATE_outputs, data, estimators, selectX, alpha, Option, CIOption)
    names_requested_ATE <- c(names_requested_ATE, "average")
  }

  # Name the outputs
  names(requested_ATE_outputs) <- names_requested_ATE

  # Give proper name to the average bounds
  if (compute_average) {
    names(requested_ATE_outputs$average)[4] <- "estimatedATEbounds"
  }

  return(requested_ATE_outputs)
}
